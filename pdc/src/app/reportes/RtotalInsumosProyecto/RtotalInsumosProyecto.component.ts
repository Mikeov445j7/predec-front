...
@Component({
  selector: 'app-RtotalInsumosProyecto',
  templateUrl: './RtotalInsumosProyecto.component.html',
  styleUrls: ['./RtotalInsumosProyecto.component.scss']
})
export class RtotalInsumosProyectoComponent implements OnInit {
  public modulo:any;
  public reporte:any;
  public modulos:any=[];
  ...
  constructor(
    ...
    public dialogo: MatDialogRef<RtotalInsumosProyectoComponent>,
        ...
        for(let j=0; j<arry.modulos[i].insumos.length; j++){
          if(arry.modulos[i].insumos[j].idMaterial){
            let mat = {
              ...
              'TotalModulo':Number(arry.modulos[i].insumos[j].cantXmodulo)*Number(arry.modulos[i].insumos[j].cantXactiv)
            }
            materiales.push(mat);
          }
        }
      }
      this.insumos = materiales;

      for (let i=0; i<materiales.length;i++){
        let aux:any;
        aux = materiales[i];
        if(materiales[i].idMaterial === aux.idMaterial){
