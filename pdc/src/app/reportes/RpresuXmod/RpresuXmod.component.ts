...
@Component({
  selector: 'app-RpresuXmod',
  templateUrl: './RpresuXmod.component.html',
  styleUrls: ['./RpresuXmod.component.scss']
})
export class RpresuXmodComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogo: MatDialogRef<RpresuXmodComponent>,
  ...
  ngOnInit() {
    ...
    this.repServ.RpresuXmod(this.data).subscribe(r=>{
      this.reporte = r;
      console.log(this.reporte);
      this.modulos = this.reporte.modulos;

      for (let i=0; i<this.modulos.length; i++){
        //console.log(this.modulos[i].modulo.nombre);
        let total = 0;
        for(let j=0; j<this.modulos[i].listadeActiv.length; j++){
         // console.log(this.modulos[i].listadeActiv[j].parcial);
          total = total + this.modulos[i].listadeActiv[j].parcial;
        }
        this.modulos[i].parcial = total.toFixed(2);
        this.reporte.modulos[i].parcial = total.toFixed(2)
       // console.log(this.reporte.modulos[i]);
      }
